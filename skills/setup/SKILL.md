...
帮第一次用这个工作区的人，把要用的集成一个个打通。实现是 `scripts/setup.sh`（wizard / guide / status / init / doctor / set），你（Agent）优先跑 `wizard --dry-run` 盘点，再按用户选择拿凭证、调脚本落盘。
...
```bash
bash scripts/setup.sh wizard --dry-run  # 非破坏式盘点：当前缺什么、每项如何配置
bash scripts/setup.sh wizard            # 交互式一键引导：初始化、体检、逐项写入 .env.local
```
...
bash scripts/setup.sh doctor     # 逐集成体检
```
...
| 腾讯会议（个人版·录制/AI纪要/逐字稿） | `tencent-meeting` | 无需密钥（OAuth） | `npm i -g @tencentcloud/tmeet` + `tmeet auth login` 扫码登录 |
...
- **工具型（飞书/公众号/小宇宙）**：给出要在他自己终端跑的命令（登录/扫码），这些交互式步骤让用户自己执行；你只负责给清晰步骤并在之后体检。
...
把 doctor 结果讲给用户听，对 ⚠️ 项给出下一步。配好后可挑一个试跑（如 `bash skills/blog/scripts/pull-blog.sh`、`scripts/pull-feishu.sh`），确认能落盘到 `sources/`。

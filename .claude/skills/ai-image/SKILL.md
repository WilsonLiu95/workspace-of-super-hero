...
通过一个 **OpenAI 兼容**的图片 endpoint 生成图片。实现是**共享脚本** `scripts/generate-image.py`（只用 Python 标准库、无三方依赖），Codex 也能直接调（见根 `AGENTS.md`）。

> **这是模板版本，不含任何密钥或私有地址。** 配置走环境变量，安全可分享。
> 用前在仓库根配置凭证（统一走 `.env.local`，已被 `.gitignore` 忽略）：
>
> ```bash
> cp .env.example .env.local        # 编辑填入 AIPROXY_API_KEY / AIPROXY_BASE_URL
> set -a; . .env.local; set +a       # 或直接 export
> ```
...
```bash
python3 scripts/generate-image.py "<prompt>"
```

默认行为：
1. 用主模型（默认 `gpt-image-2`）POST 到 `${AIPROXY_BASE_URL}/v1/images/generations`，`response_format=b64_json`。
2. 主模型返回 429/5xx → 自动改用 `gemini-3.1-flash-image` 重试一次。
3. 用 `AIPROXY_ENDPOINT` 配多个完整 URL（逗号分隔）时，前一个网络错误/5xx 会降级到下一个。
...
- 缺 `AIPROXY_API_KEY` / `AIPROXY_BASE_URL` → 退出码 2，提示先配置。

## 接口约定（OpenAI 兼容）

`POST ${AIPROXY_BASE_URL}/v1/images/generations`，Header `Authorization: Bearer {AIPROXY_API_KEY}`。请求体：

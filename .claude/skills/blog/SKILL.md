...
```bash
.claude/skills/blog/scripts/pull-blog.sh                  # 读 .env.local 的 BLOG_FEEDS
.claude/skills/blog/scripts/pull-blog.sh https://x.com/feed  # 直接传 URL，覆盖 BLOG_FEEDS
.claude/skills/blog/scripts/pull-blog.sh https://someone.dev # 传主页，自动发现 feed
TENANT=foo .claude/skills/blog/scripts/pull-blog.sh         # 额外叠加 .env.foo.local
.claude/skills/blog/scripts/pull-blog.sh --dry-run https://x.com/feed  # 只演练不写盘
```
